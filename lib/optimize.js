  ...
  buildWConfig({ iEnv, ctx, yConfig }) {
    ...
    if (yConfig.seed) {
      ...
      if (!fs.existsSync(wConfigProfixPath)) {
        throw `${LANG.OPTIMIZE.WCONFIG_TYPE_NOT_EXISTS}: ${yConfig.seed}`
      }
    }
    ...
    const r = iWconfig(yConfig, iEnv)
    if (cache.root) { // 项目本地 config
      let wPath = util.path.join(cache.root, 'webpack.config.js')

      // 配置中的 wConfig
      if (yConfig.webpackConfigPath) {
        wPath = util.path.resolve(cache.root, yConfig.webpackConfigPath)
      }
      if (fs.existsSync(wPath)) {
        const pjWConfig = require(wPath)
        if (typeof pjWConfig === 'function') {
          return webpackMerge(r, pjWConfig({ env: iEnv, config: yConfig }))
        } else {
          return webpackMerge(r, pjWConfig)
        }
      }
      return r
    } else {
...
const task = {
  async preCheck ({ iEnv, iRes, yConfig }) {
    if (!cache.root) {
      return
    }
    // + 版本校验
    ...
    if (!yConfig.seed) {
      throw LANG.OPTIMIZE.SEED_NOT_SET
    }
    if (yConfig.seed) {
      ...
      if (fs.existsSync(seedDir)) {
        ...
        throw `${LANG.OPTIMIZE.SEED_NOT_EXISTS}: ${yConfig.seed}`
      }
    }
    // - 版本校验
    // + tsconfig rewrite
  ...
  all({ iEnv, config, yConfig }) {
    const iRes = new SeedResponse()
    iRes.trigger('clear', [])

    task.preCheck({ iEnv, iRes, yConfig }).then(() => {
      requireWebpack(iRes)
  ...
  watch({ iEnv, config, yConfig }) {
    ...
    task.preCheck({ iEnv, iRes, yConfig }).then(() => {
      ...
      if (config.devServer) { // 配置了 devServer
        ...
        const devServer = new DevServer(compiler, Object.assign(devOption, config.devServer))
...
const wOpzer = function ({ config, root, iEnv, ctx }) {
  ...
  Object.keys(USAGE).forEach((key) => {
    opzer[key] = function() {
      return task[key]({ iEnv, config: wConfig, yConfig })
    }
  })
