...
const fn = {
  ...
  envInit({ iEnv, yConfig }) {
    ...
    rEnv.staticRemotePath =
      rEnv.remote || rEnv.isCommit || rEnv.proxy
        ? yConfig.commit.staticHost || yConfig.commit.hostname
        : '/'
    rEnv.mainRemotePath =
      rEnv.remote || rEnv.isCommit || rEnv.proxy
        ? yConfig.commit.mainHost || yConfig.commit.hostname
        : '/'
    return rEnv
  },
  buildWConfig({ iEnv, ctx, yConfig, root }) {
    ...
    if (yConfig.seed) {
      ...
      if (!fs.existsSync(wConfigProfixPath)) {
        throw new Error(
          `${LANG.OPTIMIZE.WCONFIG_TYPE_NOT_EXISTS}: ${yConfig.seed}`
        )
      }
    }
    ...
    if (root) {
      ...
      if (fs.existsSync(wPath)) {
        let pjWConfig = require(wPath)
        if (typeof pjWConfig === 'function') {
          pjWConfig = pjWConfig({ env: iEnv, config: yConfig })
        }
  ...
  initCompilerLog({ compiler, iRes, iEnv }) {
    ...
    compiler.hooks.done.tap('yyl-seed', (stats) => {
      ...
      statsInfo.warnings.forEach((er) => {
        iRes.trigger('msg', [
          'warn',
          iEnv.logLevel === 2 ? er : er.message || er
        ])
      })
    ...
    if (yConfig.seed) {
      ...
      if (fs.existsSync(seedDir)) {
        ...
        if (fs.existsSync(pkgPath)) {
          ...
          iRes.trigger('msg', [
            'success',
            LANG.OPTIMIZE.CHECK_TARGET_PKG_FINISHED
          ])
        }
      } else {
  ...
  async initDevServer({ compiler, wConfig, iEnv, yConfig, iRes, root }) {
    const noInfo = +iEnv.logLevel === 2 ? false : true
    const writeToDisk =
      iEnv.remote || iEnv.isCommit || iEnv.writeToDisk ? true : false
    const publicPath = (() => {
    ...
    iRes.trigger('msg', [
      'info',
      `${LANG.OPTIMIZE.WRITE_TO_DISK}: ${chalk.yellow(writeToDisk)}`
    ])
    if (!writeToDisk) {
      iRes.trigger('msg', [
        'success',
        `${LANG.OPTIMIZE.MOMERY_ADDRESS}: ${chalk.green(
          util.path.join('/webpack-dev-server')
        )}`
      ])
    }
    ...
    if (wConfig.devServer) {
      ...
      if (needInstalls.length) {
        iRes.trigger('msg', [
          'error',
          `${
            LANG.OPTIMIZE.WEBPACK_DEV_SERVER_NEED_PRE_INSTALL
          }: ${needInstalls.map((name) => chalk.yellow(name)).join(', ')}`,
          yConfig.yarn
            ? `${chalk.yellow(`yarn add ${needInstalls.join(' ')} -D`)}`
      ...
      const serverPort = iEnv.port || yConfig.localserver.port
      ...
      devServer.listen(serverPort, (err) => {
        if (err) {
          iRes.trigger('msg', [
            'error',
            LANG.OPTIMIZE.DEV_SERVER_START_FAIL,
            err
          ])
        } else {
          iRes.trigger('msg', [
            'success',
            LANG.OPTIMIZE.DEV_SERVER_START_SUCCESS
          ])
        }
      })
    } else if (cache.app) {
