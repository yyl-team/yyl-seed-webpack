    ...
    return (err, stats) => {
      if (err) {
        ...
        compilation.warnings.forEach((warn) => {
          iRes.trigger('msg', ['warn', warn.message || warn.details || warn])
        })
      }
...
const task = {
  ...
  /** 代码压缩 */
  o({ iEnv, config, yConfig, root, iRes, ctx }) {
    iEnv.isCommit = true
    config = fn.buildWConfig({ iEnv, config, ctx, root, yConfig })
    return task.all({ iEnv, config, yConfig, root, iRes })
  },
  /** watch */
  w({ iEnv, config, yConfig, root, iRes }) {
    return task.watch({ iEnv, config, yConfig, root, iRes })
  },
  /** 本地开发 */
  d({ iEnv, config, yConfig, root, iRes, ctx }) {
    iEnv.proxy = true
    iEnv.hmr = true
    config = fn.buildWConfig({ iEnv, config, ctx, root, yConfig })
    return task.watch({ iEnv, config, yConfig, root, iRes })
  ...
  /** 远程调试 */
  r({ iEnv, config, yConfig, root, iRes, ctx }) {
    iEnv.proxy = true
    iEnv.hmr = true
    iEnv.remote = true
    config = fn.buildWConfig({ iEnv, config, ctx, root, yConfig })
