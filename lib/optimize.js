...
function requireWebpack (iRes) {
  if (!webpack) {
    ...
    iRes.trigger('success', ['load webpack finished']);
  }
  return webpack;
}
...
const task = {
  async preCheck (iEnv) {
    ...
    // base config
    const basePkgPath = path.join(__dirname, '../config/base/package.json');
    if (fs.existsSync(basePkgPath)) {
      await extOs.installPackage(basePkgPath, {
        loglevel: iEnv.silent ? 'silent' : 'info'
      });
    }
