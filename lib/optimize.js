  ...
  buildWConfig(iEnv) {
    ...
    if (config.seed) {
      ...
      if (!fs.existsSync(wConfigProfixPath)) {
        throw `${LANG.OPTIMIZE.WCONFIG_TYPE_NOT_EXISTS}: ${config.seed}`
      }
    }
    ...
    if (cache.root) { // 项目本地 config
      ...
      if (fs.existsSync(wPath)) {
        ...
        if (typeof pjWConfig === 'function') {
          return webpackMerge(r, pjWConfig({ env: iEnv, config }))
        } else {
  ...
  async preCheck ({ iEnv, iRes }) {
    ...
    if (config.seed) {
      ...
      if (fs.existsSync(seedDir)) {
        ...
        throw `${LANG.OPTIMIZE.SEED_NOT_EXISTS}: ${config.seed}`
      }
    }
    // - 版本校验
    // + tsconfig rewrite
