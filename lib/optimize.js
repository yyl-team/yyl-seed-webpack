...
const fn = {
  ...
  initDevServer ({ compiler, wConfig, iEnv, yConfig, iRes, root }) {
    ...
    const writeToDisk = iEnv.remote || iEnv.isCommit ? true : false
    const devOption = {
      noInfo,
      reporter() {},
      publicPath: (() => {
    ...
    if (wConfig.devServer) { // 配置了 devServer
      ...
      cache.app.use('/webpack-dev-server', (req, res) => {
        ...
        const outputPath = middleware.getFilenameFromUrl(devOption.publicPath || '/')
        const filesystem = middleware.fileSystem
    
        writeDirectory(devOption.publicPath || '/', outputPath)
